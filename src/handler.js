...
const getAllBooksHandler = (request, h) => {
  ...
  if (name) {
    const lowName = Bookself.filter((n) =>
  ...
  if (reading === "1") {
    const response = h.response({
      status: "success",
      data: {
        books: Bookself.filter((r) => r.reading === true).map((books) => ({
          id: books.id,
          name: books.name,
          publisher: books.publisher,
        })),
  ...
  if (finished === "1") {
    const response = h.response({
      status: "success",
      data: {
        books: Bookself.filter((f) => f.finished === true).map((books) => ({
          id: books.id,
          name: books.name,
          publisher: books.publisher,
        })),
  ...
  if (reading === "0") {
    const response = h.response({
      status: "success",
      data: {
        books: Bookself.filter((r) => r.reading === false).map((books) => ({
          id: books.id,
          name: books.name,
          publisher: books.publisher,
        })),
  ...
  if (finished === "0") {
    const response = h.response({
      status: "success",
      data: {
        books: Bookself.filter((f) => f.finished === false).map((books) => ({
          id: books.id,
          name: books.name,
          publisher: books.publisher,
        })),
